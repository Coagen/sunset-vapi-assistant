      ...
      <div className="w-[300px] bg-brand-navy/95 backdrop-blur-xl border border-brand-gold/40 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
          ...
          {status === "active" && (
            <Activity
              className={`w-4 h-4 text-brand-gold ${
                volumeLevel > 0.05 ? "animate-bounce" : "opacity-50"
              }`}
            />
          )}
        </div>
        ...
        >
          ...
          {transcript.map((line, i) => {
            const isAgent =
              line.startsWith("Agent:") || line.startsWith("System:");
            ...
            if (isSystem) {
              return (
                <div
                  key={i}
                  className="text-center text-[10px] text-white/30 uppercase tracking-widest my-2"
                >
                  {text}
                </div>
              );
